...
app.get('/', function(req, res){
  articleProvider.findAll(function(error, docs){
    if(error) {
      res.send(util.inspect(error), 500);
      return;
    }
...
app.post('/blog/new', function(req, res) {
  ...
  req.assert('slug', 'only a-z, 0-9 and hyphen can be used').is(/^[a-z0-9\-]*$/).not(/^new$/);
...
app.get('/blog/:id', function(req, res) {
  ...
  if(id.match(/^\d+$/)) {
    ...
    articleProvider.findBySlug(id, callback);
  }
});
